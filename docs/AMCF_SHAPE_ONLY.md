...
A primary threshold is generated only when the seed set is exactly `{101,202,303}`, all three convergence audits pass, and every seed has identical calibration timestamps and row indices. Missing/nonconverged/misaligned input hard-fails rather than producing a normal primary threshold. Evidence is averaged across the three seeds. The cleanStatic calibration artifact stores the exact common ordered identity, each seed raw score, LOO p-value, and LOO evidence. Its threshold digest binds source commit, score bank, row index, and raw evidence; threshold scalar fields alone are never trusted. q99 and q995 are `method="higher"` quantiles of the cleanStatic `[340,410)` calibration LOO ensemble evidence. Verifier identity, row order, source intervals, q99/q995, strict alarms, metrics, and GO semantics remain exact. Only recomputed conformal p-values, evidence, and their ensemble use explicit `rtol=0, atol=1e-14`; this admits cross-NumPy arithmetic roundoff (observed at most `8.88e-16`) while rejecting meaningful `1e-10` drift and is far below every decision-threshold margin. The tolerance is persisted in config, provenance, numerical parity, and README. Alarms are always recomputed as strict `score > threshold`; saved alarm columns are not trusted. q99 is GO-primary and q995 is diagnostic. This is verifier QA, not detection-threshold or model retuning.
...
### Frozen B0 Exact comparator protocol

B0 Exact is not recalibrated by this campaign. The protected `artifacts/cmte_a2_texbat_epochfix` threshold/config/provenance hashes are verified, and its frozen `primary.B0-Exact` q99/q995 value `1.7035537524611113`, fitted from cleanStatic `[300,330)`, is reused exactly. The Shape-Only runner must never refit B0 on `[340,410)`. B0 ROC-AUC is nevertheless recomputed from B0 scores on the same common timestamps and actual phase population as AMCF. Because AMCF `[340,410)` and B0 `[300,330)` calibration protocols differ, threshold-dependent detection/FPR comparisons have an explicit limitation and cannot be interpreted as a matched operating-point comparison.

## Paired inference and collapse audit

Every Complex/comparator contrast is joined on common timestamps before analysis. Paired bootstrap resamples labels, timestamps, Complex scores, and comparator scores with the **same row indices** from the same 10 s timestamp blocks. Fewer than 2000 replicates hard-fails. ROC-AUC first selects exactly stable-pre plus wholly-post rows (transition rows are excluded), matching the scenario ROC population, and resamples only those aligned labelled rows; post detection and stable-pre FPR first select the declared phase using actual source-interval masks and then construct/resample blocks within that phase (phase-local resampling). Representation-specific thresholds are applied after each paired draw. Delta sign is **Complex - comparator**. Frozen metrics are ROC-AUC, post detection, and stable-pre FPR. A positive delta always favors Complex for ROC-AUC/post detection; stable-pre FPR interpretation retains the signed definition.
...
DS4 is explicitly `NA` in config, README, and `scenario_metrics.csv` because it is a mixed-producer sensitivity dataset. It is not one of the five attack GO scenarios and cannot enter any criterion. Complex/Magnitude EPL is an auxiliary output for every scenario at q99 and q995, including per-seed and plot-ready evidence; paired comparisons remain restricted to the requested primary Complex all9 contrasts.
...
`scripts/run_amcf_shape_only.py` is the sole end-to-end entry point. Primary mode hard-freezes baseline SHA `ef36f265fca856914b7c79b77fd0a2f85f89b4d1`, minimum valid rows `5`, seeds/settings, and has no primary `--baseline` or `--min-valid-rows` override. It requires a clean committed tree, verifies all six canonical SHA-256 pins, performs a real CUDA tensor probe, reads only the six allowlisted NPZ fields (never C/N0), writes through an atomic staging directory, and refuses overwrite. Before publication it runs the independent summarizer/verifier against staging and requires exact decision agreement; only then does one atomic rename publish the final directory. Its only primary destination is exactly `artifacts/amcf_r1_shape_only/`. A synthetic `--smoke` mode must use another destination and cannot create the final result. The too-small-P gate is `abs(P) >= max(clean-train q0.005 higher, float64 positive floor)` together with the explicit `abs(P) > 0` quality rule, so an exact-zero Prompt is rejected even when the empirical quantile is zero.
...
`scripts/summarize_amcf_shape_only.py` defaults to publication verification and therefore fails closed without CUDA. It verifies `hashes.json` before parsing evidence, independently rebuilds canonical features, scalers, causal examples, and fit manifests; loads each real Torch checkpoint with strict architecture/metadata checks; re-infers every persisted scenario and calibration raw NLL on the same required CUDA backend and requires bit-exact score equality plus exact identity/source equality. It separately replays all 12 checkpoints on CPU as `cross_device_numerical_audit`, records finite count and absolute/relative p50/p95/p99/max summaries for every checkpoint × six scenarios and calibration, and applies a conservative `max_abs <= 2e-3` engineering fault guard selected after backend FP32 characterization. That guard is not a raw score binding tolerance and was never used to tune a model, hyperparameter, conformal threshold, or attack result.

From the independent CPU replay, the verifier recomputes every variant's calibration LOO conformal evidence and q99/q995, all strict alarm decisions, and all reported AMCF scenario metrics. CUDA-versus-CPU threshold, alarm, and metric decision changes must each be zero. Attack data participates only in this post-fit QA/semantic invariance check and is never fit or tuning input. The summarizer repeats the complete numerical audit and compares it to saved `numerical_parity.json`, rather than trusting a self-reported pass. A limited `--hash-only` mode exists for later non-GPU read-only checks, is explicitly non-publication, and cannot publish or establish score/semantic parity. q995 remains diagnostic; every attack result is exploratory/developmental.

The required inventory is `README.md`, `config.json`, `feature_schema.json`, `provenance.json`, `input_hashes.json`, `training_history.csv`, `convergence_audit.json`, `thresholds.json`, `scenario_metrics.csv`, `seed_metrics.csv`, `paired_comparisons.csv`, `decision.json`, `feature_audit.json`, `model_audit.json`, `window_qa.json`, `feature_cache/`, `per_epoch/`, `plots/`, `models/`, and self-excluding `hashes.json`; primary-full additionally requires `calibration_evidence.json` and `numerical_parity.json`. `model_audit.json` is derived by instantiating the actual model class, cross-checks all 12 checkpoint architectures, records the adapter/GRU/decoder/head parameter breakdown (Complex 19,208; Magnitude 19,012; 196 / 1.030927835% difference using the smaller denominator), and enforces the 5% fairness bound. The numerical parity and model audit hashes are bound by provenance, decision source digests, and the complete outer hash inventory.

`plots/` contains exactly six rendered PNG score figures (`cleanStatic`, `DS1`, `DS2`, `DS3`, `DS7`, `DS8`) plus optional plot-ready CSV evidence. Each PNG uses three aligned timestamp panels with separate y-scales and own q99 line for Complex all9, Magnitude all9, and B0 Exact; attack plots mark stable-pre, onset/wholly-post, and post bands. Matplotlib Agg, fixed style/size/DPI, and explicit figure closure make rendering deterministic. The verifier checks exact PNG inventory, PNG magic, and nontrivial size.

The deterministic README renderer consumes the bound full-precision artifact inputs and reports the single hypothesis, prior AMCF-R1 branch removals, fair parameter/feature counts, all 12 convergence rows, cleanStatic and attack q99 metrics, EPL auxiliary results, all paired CIs, criterion-level counts/evidence, final decision, claim limits, B0 calibration limitation, and numerical-verifier QA correction. Publication verification requires byte-identical regeneration.

## Scope of this source commit

This source commit is a verifier numerical correction after the preserved failed publication stage `artifacts/amcf_r1_shape_only.tmp-1092723`. The root cause was characterized FP32 CPU/CUDA backend drift: production CUDA checkpoint replay was exact, while CPU replay changed no conformal threshold, alarm, or reported scenario metric decisions. This correction does **not** change or tune the model, architecture, hyperparameters, checkpoint selection, conformal protocol/thresholds, or attack results. It does not mutate or publish the failed stage, does not run the full campaign, and requires the next campaign to start from this new committed source.
