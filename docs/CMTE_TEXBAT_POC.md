...
S1 uses fixed prior restart weights `w_s=2^-(s+1)` plus unstarted reserve. There is no free maximum or floor. S2 is `G=max(0,G+log(E)-drift)`. Fixed drift grid is `[0,.01,.025,.05,.1]`. Normal validation selection minimizes 20 s block-any-alarm fraction then rising-edge false-alarm events/min, ties by higher censored ARL, then fixed S2 and smaller drift. Thresholds are higher finite q99 of 20 s validation block maxima; limited block count is explicit.

False-alarm outputs have distinct schemas: `epoch_fpr`/`stable_pre_fpr` are the fraction of epochs with `score > threshold`; `alarm_epoch_occupancy_per_min` is above-threshold epoch count/min; `false_alarm_events` counts rising edges from `<= threshold` to `> threshold`, reset per recording, and `false_alarms_per_min` is events/min. `sequence_any_alarm_fraction`, first crossing, censored run length/ARL, and reset-aware block-any are separate. Alarms do not force detector reset. Normal shuffle diagnostics aggregate epochs first, then genuinely permute their order within recording while preserving the score/evidence multiset and recording reset structure; they report original/shuffled occupancy, event rate, block-any, and final/max state and are diagnostic only.
