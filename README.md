...
The `research/pg-scc-stage0-static-k9` campaign implements a frozen global K=3/5/9 complex-correlator mask and a covariance-whitened one-vs-two-source same-PRN GLRT. Design assumptions, formulas, leakage boundary, and reproduction commands are documented in [docs/PG_SCC.md](docs/PG_SCC.md); all explicit assumptions are also serialized in `configs/pg_scc_stage0_static_k9.json`.
