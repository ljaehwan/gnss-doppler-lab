"""Causal adapter from GNSS-SDR 9-tap windows to GCMR-PI ``EventRecord``.
...
``.gcmr_pi_9tap_pipeline.EventRecord``.  Tests and integrations may
...
NINE_TAPS = ("E4", "E3", "E2", "E", "P", "L", "L2", "L3", "L4")
REQUIRED_TAPS = NINE_TAPS
...
class PeakWindowRecord:
    """One finite, half-open-window 9-tap aggregate for a single PRN.
    ...
    def validate(self) -> None:
        ...
        if tuple(self.tap_names) != REQUIRED_TAPS:
            raise ValueError("only exactly the real 9-tap E4,E3,E2,E,P,L,L2,L3,L4 layout is accepted")
        epl = np.asarray(self.epl, dtype=np.float64)
        if epl.shape != (3,) or not np.isfinite(epl).all() or np.allclose(epl, 0.0):
            raise ValueError("9-tap aggregate must be a finite non-placeholder vector with shape (3,)")
...
def _validate_series(series: Any, expected_tap_names: tuple[str, ...] = REQUIRED_TAPS) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if np.any(np.all(magnitudes == 0.0, axis=0)):
        raise ValueError("9-tap columns must be real measurements, not all-zero placeholders")
...
def aggregate_peak_windows(series: Any, windows: Sequence[tuple[float, float]], *, min_epochs: int = 1) -> list[PeakWindowRecord]:
    """Aggregate a ``TrackingPeakSeries`` into explicit half-open 9-tap windows.
...
def _exact_target(records: Sequence[PeakWindowRecord], start: float, end: float, prn: str) -> PeakWindowRecord:
    ...
    if len(matches) != 1:
        raise CausalEventBuildError(f"{prn} needs exactly one 9-tap target aggregate for relation window [{start}, {end}); found {len(matches)}")
    return matches[0]


def _event_record_type() -> Callable[..., Any]:
    try:
        return importlib.import_module(".gcmr_pi_9tap_pipeline", __package__).EventRecord
...
def build_event_record(relation_event: Any, peak_windows: Mapping[int | str, Sequence[PeakWindowRecord]], *, history_window: int, event_record_type: Callable[..., Any] | None = None) -> Any:
    ...
    relation pairs must have an exact target 9-tap window and ``history_window``
    ...
    for prn in prns:
        ...
        if len(prior) < history_window:
            unavailable.append(f"{prn} has {len(prior)} valid prior 9-tap windows before {start}; needs {history_window}")
            continue
...
def index_peak_windows(series: Any, windows: Sequence[tuple[float, float]], *, min_epochs: int = 1, expected_tap_names: tuple[str, ...] = REQUIRED_TAPS) -> PeakWindowIndex:
    """Index only target and requested historical 9-tap windows for one segment."""
    if int(min_epochs) != min_epochs or min_epochs < 1:
        raise ValueError("min_epochs must be a positive integer")
...
__all__ = ["NINE_TAPS", "REQUIRED_TAPS", "CausalEventBuildError", "PeakWindowRecord", "PeakWindowIndex", "aggregate_peak_windows", "index_peak_windows", "build_event_record", "build_event_record_indexed"]
