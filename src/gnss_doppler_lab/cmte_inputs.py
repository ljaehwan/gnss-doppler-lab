...
def extract_role_innovations(nodes,model,features,mean,std,*,seq_len=12,device="cpu",extractor=None):
    """Partition canonical node windows first, then invoke B0 separately per role.

    Rewritten run IDs force extractor grouping/state reset.  Only windows fully
    contained in a role are admitted, so no history window crosses a boundary.
    """
    ...
    for role,(lo,hi) in bounds.items():
        ...
        result[role]=extract_innovations(local,model,features,mean,std,seq_len=seq_len,device=device,extractor=extractor)
    return result
...
def extract_recording_innovations(nodes,model,features,mean,std,*,scenario,seq_len=12,device="cpu",extractor=None):
    ...
    return extract_innovations(local,model,features,mean,std,seq_len=seq_len,device=device,extractor=extractor)
