...
from .gcmr_peak_innovation import (NormalOnlyCalibrator, EventDiagnostics, common_drive_statistics,
    normal_loading_threshold, relation_destruction, aggregate_event_score, binomial_tail_from_exceedances)



def normal_loading_threshold_9d(events, quantile=.99):
    loads=[]
    for z in events:
        z=np.asarray(z,float)
        if z.ndim!=2 or z.shape[1]!=9 or len(z)<2 or not np.isfinite(z).all(): raise ValueError("9D normal events require finite (N,9), N>=2")
        loads.extend(np.abs(np.linalg.svd(z,full_matrices=False)[0][:,0]))
    return float(np.quantile(loads,quantile))

def common_drive_statistics_9d(z, threshold):
    z=np.asarray(z,float)
    if z.ndim!=2 or z.shape[1]!=9 or len(z)<2 or not np.isfinite(z).all(): raise ValueError("9D event requires finite (N,9), N>=2")
    gram=z@z.T; eig=np.linalg.eigvalsh(gram); common=float(max(eig[-1],0)/(np.trace(gram)+1e-12)); loading=np.abs(np.linalg.svd(z,full_matrices=False)[0][:,0]); part=loading/max(loading.sum(),1e-12); count=int(np.count_nonzero(loading>=threshold))
    from types import SimpleNamespace
    return SimpleNamespace(n=len(z),n_eff=float(1/(part@part)),loading_count=count,at_least_four=count>=4,s_common=common)

def relation_destruction_9d(z, seed=0):
    z=np.asarray(z,float)
    if z.ndim!=2 or z.shape[1]!=9: raise ValueError("9D innovations required")
    norms=np.linalg.norm(z,axis=1); directions=z/np.maximum(norms[:,None],1e-12); perm=np.random.default_rng(seed).permutation(len(z))
    if len(z)>1 and np.array_equal(perm,np.arange(len(z))): perm=np.roll(perm,1)
    return directions[perm]*norms[:,None]

def safe_center_tap_normalize(x, eps=1e-12, min_center=1e-6):
    x=np.asarray(x,float)
    if x.ndim < 1 or x.shape[-1] != 9: raise ValueError("9D input required")
    center=x[...,4]; valid=np.isfinite(x).all(axis=-1)&(center>min_center); out=np.zeros_like(x)
    np.divide(x,np.maximum(center,eps)[...,None],out=out,where=valid[...,None]); return out,valid

class GenericConditionalInnovationWhitener:
    def __init__(self, dimension, context_edges=(35.,45.), min_bin_samples=20, regularization=1e-5, eps=1e-12):
        if dimension != 9: raise ValueError("isolated whitener is strictly 9D")
        self.dimension,self.context_edges,self.min_bin_samples,self.regularization,self.eps=dimension,np.asarray(context_edges),min_bin_samples,regularization,eps
    def _parameters(self,v):
        mean=v.mean(0); c=v-mean; cov=c.T@c/max(len(v)-1,1); cov=(1-self.regularization)*cov+self.regularization*np.diag(np.diag(cov))+np.eye(9)*self.eps; w,q=np.linalg.eigh(cov); return mean,q@np.diag(1/np.sqrt(np.maximum(w,self.eps)))@q.T
    def fit(self,x,context=None):
        x=np.asarray(x,float)
        if x.ndim not in (2,3) or x.shape[-1]!=9 or not np.isfinite(x).all(): raise ValueError("finite normal 9D innovations required")
        flat=x.reshape(-1,9);self.global_mean_,self.global_inv_sqrt_=self._parameters(flat);self.bin_parameters_={}
        if context is not None:
            c=np.asarray(context,float)
            if c.shape!=x.shape[:-1] or not np.isfinite(c).all(): raise ValueError("context mismatch")
            for b in range(3):
                v=flat[np.digitize(c.reshape(-1),self.context_edges)==b]
                if len(v)>=self.min_bin_samples:self.bin_parameters_[b]=self._parameters(v)
        self.fitted_=True;return self
    def transform(self,x,context=None):
        if not getattr(self,'fitted_',False):raise RuntimeError("fit normal 9D innovations first")
        x=np.asarray(x,float)
        if x.ndim not in (2,3) or x.shape[-1]!=9 or not np.isfinite(x).all():raise ValueError("finite 9D innovations required")
        c=np.asarray(context,float) if context is not None else None
        if c is not None and c.shape!=x.shape[:-1]:raise ValueError("context mismatch")
        bins=np.digitize(c.reshape(-1),self.context_edges) if c is not None else np.full(x.size//9,-1);flat=x.reshape(-1,9);out=np.empty_like(flat)
        for i,v in enumerate(flat):
            mean,m=self.bin_parameters_.get(int(bins[i]),(self.global_mean_,self.global_inv_sqrt_));out[i]=m@(v-mean)
...
class GCMRPeakInnovationPipeline:
    ...
    def __init__(self, window: int, hidden_size: int = 8, epochs: int = 30, learning_rate: float = .02, seed: int = 0, feature_dim: int = 3):
        ...
        self.whitener = GenericConditionalInnovationWhitener(feature_dim, min_bin_samples=2)
    ...
    def _arrays(self, event: EventRecord):
        ...
        hn, hv=safe_center_tap_normalize(h); yn, yv=safe_center_tap_normalize(raw)
        if not hv.all() or not yv.all(): raise ValueError("all 9-tap center taps must exceed the safe normalization threshold")
        return hn, yn
    ...
    def fit_normal(self, normal_train: Sequence[EventRecord], normal_validation: Sequence[EventRecord]):
        ...
        self.pairs.fit(normal_pairs); self.loading_threshold_=normal_loading_threshold_9d(zs)
    ...
    def _diagnostics(self,z,event):
        stats=common_drive_statistics_9d(z,self.loading_threshold_)
    ...
    def score_attack(self, event: EventRecord, destruction_seed: int = 0) -> EventScore:
        ...
        return EventScore(event.time,len(event.prns),d,scores,self._pair_score(relation_destruction_9d(z,destruction_seed),event))
