...
from .spoofing import SpoofingError, generate_spoofing_iq


def main(argv=None) -> int:
    parser = argparse.ArgumentParser(prog="gnss-iq", description="Generate normal or RF-level spoofed GPS L1 C/A software IQ")
    ...
    try:
        ...
        manifest = generate_spoofing_iq(cfg, runner) if cfg.spoofing else generate_iq(cfg, runner)
        print(manifest); return 0
    except (ConfigError, SimulatorError, SpoofingError, OSError, FileExistsError) as exc:
