...
OUT = ROOT / "artifacts" / "sci_ds4_soft_quorum_gate"
...
def morphology_cols(df: pd.DataFrame) -> list[str]:
    return [
        ...
        and c.startswith("tap_") and "_rel_prompt_" in c
    ]
...
def event_scores(ns: pd.DataFrame, prior_thr: float, soft_thr: float) -> pd.DataFrame:
    ns = ns.copy()
    ns["event_bin_s"] = (ns["window_mid_s"] * 2).round() / 2.0
    ns["prior_high"] = ns["node_score"] > prior_thr
    ...
    ev["prior_high_fraction"] = g["prior_high"].mean().to_numpy(dtype=float)
    ev["soft_high_fraction"] = g["soft_high"].mean().to_numpy(dtype=float)
    ev["score_prior_consensus_gate"] = ev["score_mean"] * (1.0 + PRIOR_ALPHA * ev["prior_high_fraction"])
...
def main() -> None:
    ...
    prior_thr = float(clean_ns["node_score"].quantile(PRIOR_NODE_Q))
    soft_thr = float(clean_ns["node_score"].quantile(SOFT_NODE_Q))
    clean_ev = event_scores(clean_ns, prior_thr, soft_thr)
    ds4_ev = event_scores(ds4_ns, prior_thr, soft_thr)

    score_cols = ["score_mean", "score_prior_consensus_gate", "score_soft_quorum_gate"]
    metrics = pd.DataFrame([eval_ds4(ds4_ev, clean_ev, c) for c in score_cols])
    metrics.to_csv(OUT / "ds4_soft_quorum_gate_metrics.csv", index=False)
    ds4_ev[["window_mid_s", *score_cols, "prn_count", "prior_high_fraction", "soft_high_fraction"]].to_csv(
        OUT / "ds4_event_scores.csv", index=False
    )

    prior = metrics.loc[metrics.score_col == "score_prior_consensus_gate"].iloc[0].to_dict()
    ...
    summary = {
        ...
        "protocol": {
            ...
            "auxiliary_gate": "PRN relation/geometry proxy is only a bounded soft quorum multiplier, 1 + 2.0 * fraction of tracked PRNs above the cleanStatic node q70 cutoff.",
        ...
        "features": cols,
        "prior_node_quantile": PRIOR_NODE_Q,
        "prior_node_threshold": prior_thr,
        "soft_node_quantile": SOFT_NODE_Q,
        "soft_node_threshold": soft_thr,
        "soft_alpha": SOFT_ALPHA,
        "metrics_csv": str((OUT / "ds4_soft_quorum_gate_metrics.csv").relative_to(ROOT)),
        ...
        "delta_vs_prior_consensus_gate": {
            "auc": soft["auc_pre_vs_post_buffered"] - prior["auc_pre_vs_post_buffered"],
            "q95_post_det_rate": soft["q95_post_det_rate"] - prior["q95_post_det_rate"],
            "q95_pre_fp_rate": soft["q95_pre_fp_rate"] - prior["q95_pre_fp_rate"],
            "q99_post_det_rate": soft["q99_post_det_rate"] - prior["q99_post_det_rate"],
            "q99_5_post_det_rate": soft["q99_5_post_det_rate"] - prior["q99_5_post_det_rate"],
            "q99_9_post_det_rate": soft["q99_9_post_det_rate"] - prior["q99_9_post_det_rate"],
        },
    }
