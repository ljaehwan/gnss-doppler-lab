...
def row(i,scenario="cleanStatic",prn=1,channel="a",role=None):
 d={"scenario":scenario,"prn":prn,"channel":channel,"support_start_sample":i*25000,"support_end_sample":i*25000+25000,"support_samples":25000,"cn0_db_hz":35,"carrier_lock":.95}
 if role:d["role"]=role
 return d
...
def test_normalization_floor_is_explicit():
 y,d=normalize_caf(np.zeros((11,17),complex),floor=.1);assert np.all(y==0) and bool(d["floor_applied"])
...
def test_tracker_sample_binding_fixture(tmp_path):
 ...
 with h5py.File(tracker/"epl_tracking_ch_0.mat","w") as f:
  for name,value in {"PRN_start_sample_count":np.arange(22)*25000,"PRN":np.ones(22),"CN0_SNV_dB_Hz":np.ones(22)*35,"carrier_lock_test":np.ones(22)*.9}.items():f[name]=value
...
def test_wls_h1_le_h0_and_second_source_sensitivity():
 rng=np.random.default_rng(4);t0=rng.normal(size=(4,5))+1j*rng.normal(size=(4,5));td=rng.normal(size=(4,5))+1j*rng.normal(size=(4,5));variance=np.ones_like(t0.real)
 plain=two_source_wls(t0,t0,{(0,0):td},variance);mixed=two_source_wls(t0+.6j*td,t0,{(0,0):td},variance)
...
def test_variance_and_threshold_are_normal_only():
 rng=np.random.default_rng(3);v=learn_diagonal_variance(rng.normal(size=(10,2,3))+1j*rng.normal(size=(10,2,3)));assert v.shape==(2,3) and np.all(v>0)
...
def test_timeline_mapping():
 ...
 assert m.phase_name("ds4",128.22)=="post_onset" and m.phase_name("ds8",150)=="ge_150"
...
def test_fail_closed_never_invokes_attack_scoring_callback():
 ...
 audits={s:{"matches_expected":True,"counts":m.EXPECTED_PHASE[s]} for s in ("ds3","ds4","ds7","ds8")}
 result=m.foundation_gate(audits,forbidden);assert result["verdict"]=="FOUNDATION_INVALID" and result["no_attack_raw_scoring_performed"] and not called
...
def _artifact(root):
 ...
 for n in ("normal_model_summary.json","thresholds.json","bootstrap_results.json"):m.dump(root/n,{"status":"NOT_EVALUATED","reason":reason})
 ...
 m.dump(root/"verification_report.json",{"status":"PENDING_INDEPENDENT_VERIFICATION"})
 files={str(p.relative_to(root)):m.digest(p) for p in root.iterdir() if p.is_file() and p.name not in {"checksums.json","verification_report.json"}};m.dump(root/"checksums.json",{"algorithm":"sha256","files":files})
...
def test_independent_verifier_and_checksum_tamper(tmp_path):
 ...
 first=v.verify(root,recompute_external=False);assert first["status"]=="PASS",first
