...
RUN_ID = "texbat-ds4-static-matched-power-position-push"
RAW = Path("data/external/texbat/raw/ds4.bin")
OUT = Path("artifacts/texbat_ds4_detection_normal_v3_large_50")
RECEIVER_DIR = OUT / "receiver" / RUN_ID
FEATURE_CSV = OUT / "tracking_features.csv"
MODEL_DIR = Path("artifacts/conditional_integrated_gru_normal_v3_large_50")
...
def run_receiver_if_needed(force: bool = False) -> Path:
    ...
    doc = {
        ...
        "source": {"dataset": "TEXBAT", "scenario_id": "ds4", "iq": str(RAW), "iq_sha256": raw_sha, "sample_rate_hz": SAMPLE_RATE, "sample_format": "ishort_complex_iq"},
...
def build_texbat_features(force: bool = False):
    ...
    if force or not FEATURE_CSV.exists():
        export_tracking_feature_dataset([RECEIVER_DIR], output_path=FEATURE_CSV, manifest_output_path=FEATURE_CSV.with_suffix(".manifest.json"), window_s=1.0, stride_s=0.5, min_epochs=4, label="texbat_ds4_spoofing")
...
def score_texbat(node_csv: Path, graph_csv: Path):
    ...
    (OUT / "texbat_ds4_detection_summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return summary
...
def main():
    ...
    node_csv, graph_csv = build_texbat_features(force="--force" in sys.argv)
