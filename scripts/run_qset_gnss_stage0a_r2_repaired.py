#!/usr/bin/env python3
"""R2 CLI with the pre-result receiver-build path repair."""
...
from gnss_doppler_lab.qset_stage0a_r2_execution_repair import build_receiver_repaired, clean_execution_repaired, freeze_clean_artifacts_repaired
...
def main() -> int:
    ...
    if args.command == "build-receiver": result = build_receiver_repaired()
    elif args.command == "run-clean":
        full = clean_execution_repaired(); freeze_clean_artifacts_repaired(full); result = {"status": "PASS_CLEAN_EXECUTION", "clean_status": full["clean"]["status"], "receiver_sha256": full["receiver_build"]["receiver_sha256"], "threshold_sha256": full["clean"]["threshold_sha256"]}
