...
def main() -> int:
    ...
    dump_json(
        "preregistration.json",
        {
            ...
            "receiver_repair": ["retain R2 explicit scalar serialization", "fix channel-to-PRN map", "load pre-onset frozen PRN/Doppler/first-raw-interval handoff", "use the source-audited 30 s cleanStatic or 5 s DS/OS replay-relative target", "fail closed if target raw interval is missed or a later reacquisition tries to reuse the past target"],
            "phase_b_handoff_procedure_if_authorized": "Reuse only the already frozen pre-onset family handoffs: TEXBAT cleanStatic uses its 0 s clean handoff and 30 s target; DS3 and DS7 use the TEXBAT DS3 handoff derived from the original 90 s pre-onset R2 slice with a 5 s target; OAKBAT cleanStatic, OS3, and OS4 use the OAKBAT OS3 handoff derived from the original 90 s pre-onset R2 slice with a 5 s target. All Phase-B configs are hashed before the repaired Phase A. No new acquisition pass, post-onset row, physical TRACE value, or attack metric may enter handoff selection.",
