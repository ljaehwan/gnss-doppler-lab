#!/usr/bin/env python3
"""Independent verifier for the PG-SCC R2 repair follow-up."""
...
DEFAULT_ARTIFACT = ROOT / "artifacts/pg_scc_stage0_r2_repair_followup"
BRANCH = "research/pg-scc-stage0-r2-repair-followup"
BASE_SHA = "05cb5a5421461bb90bafc6cd3683a119ddf09555"
...
PREDECESSOR_RUN_STATE_SHA256 = "3ccf0e3ffa91d9c6907dc137917b4de9f00f03298f6e06ea46453fee97c632b3"
PLOTS = (
...
def verify_preexecution(root: Path, implementation_sha: str) -> dict[str, Any]:
    ...
    if errors:
        return {"schema": "pg_scc_stage0_r2_repair_followup_preexecution.v1", "status": "FAIL", "errors": errors}
    ...
    if predecessor.get("authorized_failure_metadata_only", {}).get("protected_run_state_sha256") != PREDECESSOR_RUN_STATE_SHA256:
    ...
    for line in _git(repo, "status", "--porcelain=v1", "--untracked-files=all").splitlines():
        relative = line[3:]
        if relative.startswith("artifacts/pg_scc_stage0_r2_repair_followup/"):
            continue
        dirty.append(line)
    if dirty:
    ...
    return {
        ...
        "schema": "pg_scc_stage0_r2_repair_followup_preexecution.v1",
...
def verify_tree(root: Path) -> dict[str, Any]:
    ...
    errors.extend(f"missing:{name}" for name in sorted(required) if not (root / name).is_file())
    if errors:
        return {"schema": "pg_scc_stage0_r2_repair_followup_verification.v1", "status": "FAIL", "errors": sorted(set(errors))}
    prereg = _load(root / "preregistration.json")
    predecessor = _load(root / "predecessor_failure_binding.json")
    pre_run = _load(root / "pre_run_state.json")
    semantic = _load(root / "semantic_diff_audit.json")
    ...
    if metadata.get("protected_attempt_count") != 1 or metadata.get("protected_run_state_sha256") != PREDECESSOR_RUN_STATE_SHA256:
        errors.append("predecessor_binding")
    if semantic.get("status") != "PASS" or not semantic.get("non_plot_scientific_functions_equivalent"):
        errors.append("semantic_diff")
    ...
    return {
        ...
        "schema": "pg_scc_stage0_r2_repair_followup_verification.v1",
...
def verify_fresh_clone(root: Path, revision: str) -> dict[str, Any]:
    ...
    temp = Path(tempfile.mkdtemp(prefix="pg-scc-r2-repair-verify."))
    try:
        clone = temp / "clone"
        ...
        command = [
            sys.executable, str(clone / "scripts/verify_pg_scc_r2_repair_followup.py"),
            "--artifact", str(clone / "artifacts/pg_scc_stage0_r2_repair_followup"),
        ]
