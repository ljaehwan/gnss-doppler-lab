...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="gcspo-fresh-clone-") as temporary:
        ...
        tests = sorted(str(path.relative_to(checkout)) for path in (checkout / "tests").glob("test_gcspo*.py"))
        ...
        subprocess.run([str(python), "scripts/verify_gcspo_stage0.py", "--mode", "full",
                        "--artifact-dir", str(cloned_artifact)], cwd=checkout, check=True)
        clean_reproduction = "NOT_PERMITTED_OR_SOURCE_ABSENT"
        if args.clean_root is not None and args.clean_root.is_dir():
        ...
        manifest_sha = sha256_file(cloned_artifact / "artifact_manifest_sha256.json")
        report = {"schema": "gnss-doppler-lab.gcspo-stage0.fresh-clone-verifier-report.v2",
                  "commit": args.commit, "target_commit": args.commit, "evidence_commit": clone["head"],
