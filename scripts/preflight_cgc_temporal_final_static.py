#!/usr/bin/env python3
"""Select untouched phase-randomized final-test geometries using LOS only."""
...
CONFIG = ROOT / "configs/experiments/cgc_temporal_final_static_pool_v1.json"
PROTOCOL = ROOT / "docs/results/cgc_temporal_final_static_preflight_protocol_v1.md"
OUTPUT = ROOT / "artifacts/cgc_temporal_final_static_preflight_v1"
RELEASE_INPUTS = (
    "configs/experiments/cgc_temporal_final_static_pool_v1.json",
    "docs/results/cgc_temporal_final_static_preflight_protocol_v1.md",
    "scripts/preflight_cgc_temporal_final_static.py",
)
...
def validate(config: dict[str, Any]) -> None:
    if config.get("schema") != "gnss-doppler-lab.cgc-temporal-final-static-pool":
    ...
    contract = (
        preflight["duration_seconds"], preflight["sample_rate_hz"],
        preflight["minimum_startup_los_prns"],
    )
    if contract != (1, 1_000_000, 10):
    ...
    for slot in preflight["slot_order"]:
        ...
        for row in rows:
            norm = sum(float(value) ** 2 for value in row["target_offset_enu_m"]) ** 0.5
            if abs(norm - 100.0) > 1e-9:
...
def probe(
    config: dict[str, Any], candidate: dict[str, Any], scratch: Path, logs: Path,
) -> dict[str, Any]:
    ...
    rf_config = RFGenerationConfig(
        ...
        scenario=Scenario(
            ...
            StaticPosition(
                float(position["latitude_deg"]), float(position["longitude_deg"]),
...
def run(output: Path) -> Path:
    ...
    logs = output / "logs"
    logs.mkdir()
    with tempfile.TemporaryDirectory(prefix="cgc-temporal-final-support-") as directory:
    ...
    for slot in config["preflight"]["slot_order"]:
        eligible = [
            ...
            if row["slot"] == slot and by_id[row["candidate_id"]]["support_eligible"]
        ]
        if not eligible:
            raise RuntimeError(f"no support-eligible candidate in {slot}")
        selected.append(eligible[0])
    summary = {
        "schema": "gnss-doppler-lab.cgc-temporal-final-static-preflight",
        "schema_version": 1,
