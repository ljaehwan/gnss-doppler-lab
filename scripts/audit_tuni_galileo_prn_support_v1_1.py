#!/usr/bin/env python3
"""Corrected exhaustive fixed-PRN support audit for TUNI Galileo.

Version 1.0 could not connect its flowgraph because GNSS-SDR v0.0.19 rejects
36 Galileo E1 channels.  This technical correction covers the identical PRN
roster with two non-overlapping 18-channel banks.  It remains an input-support
diagnostic and never recomputes the released detector result.
"""
...
PRN_BANKS = (tuple(range(1, 19)), tuple(range(19, 37)))
EXPECTED_RECEIVER_SHA256 = "3deb86bd586fe1db3ff681f8e67d357cbf6f32a3cba26258642d1ab9e79a3bdb"
RELEASE_TOKEN = "RELEASE-TUNI-GALILEO-PRN-SUPPORT-AUDIT-V1-1"


def _load_preflight() -> Any:
    path = ROOT / "scripts" / "preflight_tuni_galileo_clean.py"
    spec = importlib.util.spec_from_file_location("tuni_clean_preflight_support_audit_v1_1", path)
...
def render_fixed_prn_config(
    *, iq_path: Path, output_dir: Path, duration_s: float, prns: tuple[int, ...]
) -> str:
    if not prns or len(set(prns)) != len(prns) or not all(1 <= prn <= 36 for prn in prns):
        raise ValueError("PRN bank must contain unique Galileo PRNs 1 through 36")
    text = PREFLIGHT.render_config(
        iq_path=iq_path,
        output_dir=output_dir,
        input_samples=PREFLIGHT.ishort_source_item_count(duration_s, 50_000_000),
        channel_count=len(prns),
        tracking_tap_count=9,
        input_rate_hz=50_000_000,
        internal_rate_hz=12_500_000,
    )
    assignments = "\n".join(f"Channel{index}.satellite={prn}" for index, prn in enumerate(prns))
...
def channel_support(mat_paths: list[Path]) -> list[dict[str, int]]:
    ...
    for path in mat_paths:
        ...
        if not valid:
            continue
        for prn in sorted(set(valid)):
            rows.append({
                "channel": int(path.stem.rsplit("_", 1)[-1]),
                "prn": prn,
                "valid_epochs": valid.count(prn),
            })
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=ROOT / "artifacts" / "tuni_galileo_prn_support_audit_v1_1",
    )
...
def main() -> int:
    ...
    if args.duration_s != 10.0:
        raise ValueError("v1.1 audit duration is frozen at 10.0 seconds")
    ...
    for scenario_id, (relative, spoofed) in SCENARIOS.items():
        ...
        for bank_number, prns in enumerate(PRN_BANKS, start=1):
            bank_dir = scenario_dir / f"bank_{prns[0]}_{prns[-1]}"
            raw_dir = bank_dir / "raw"
            raw_dir.mkdir(parents=True)
            config_path = bank_dir / "receiver.conf"
            config_path.write_text(
                render_fixed_prn_config(
                    iq_path=iq_path, output_dir=bank_dir, duration_s=args.duration_s, prns=prns
                ),
                encoding="utf-8",
            )
            command = [str(executable), f"--config_file={config_path}", "--keyboard=false"]
            completed = subprocess.run(
                command, cwd=bank_dir, capture_output=True, text=True,
                timeout=args.timeout_s, check=False,
            )
            (bank_dir / "receiver.log").write_text(completed.stdout + completed.stderr, encoding="utf-8")
            bank_support = channel_support(sorted(raw_dir.glob("epl_tracking_ch_*.mat")))
            for row in bank_support:
                row["bank"] = bank_number
            support.extend(bank_support)
            ...
            if completed.returncode != 0:
                raise RuntimeError(f"receiver failed for {scenario_id} bank {bank_number}")
        ...
        print(f"[tuni-prn-support-v1.1] {scenario_id} complete", flush=True)

    summary = {
        "schema": "gnss-doppler-lab.tuni-galileo-prn-support-audit.v1_1",
        "scope": "receiver-input support audit; released detector result is not recomputed",
        "duration_seconds": args.duration_s,
        "fixed_prn_banks": [list(bank) for bank in PRN_BANKS],
