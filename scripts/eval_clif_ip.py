...
COMPONENT_SPECS={"Full":["B0","M1","P3","concordance"],"minus_M1":["B0","P1"],"minus_B0history":["M1","P2"],"minus_concordance":["B0","M1","P3"]}
...
def epoch_scores(d,models,whiteners,lag,refs=None,calibrations=None):
 ...
 m=d.groupby("t").agg(M1=("M1_score","first"),m1_available_s=("m1_available_s","first"),tracked=("prn","count")).reset_index();o=o.merge(m,on="t");o["available_s"]=o[["available_s","m1_available_s"]].max(axis=1);o["B0"]=o.P0_median;o["P1"]=o.P1_median;o["P2"]=o.P2_median;o["P3"]=o.P3_median
...
def permutation_summary(observed,distribution,confidence=.95):
 ...
 return {"observed_mean_delta":float(observed),"p_value":float(min(1,(1+two)/(len(d)+1))),"p_value_resolution":float(1/(len(d)+1)),"ci_low":float(np.quantile(d,a)),"ci_high":float(np.quantile(d,1-a)),"ci_definition":"quantiles of permutation delta distribution (not a sampling-confidence interval)","repetitions":int(len(d))}
...
def main():
 ...
 bpaths,mpaths=paths(out);ckpath=out/"clif_b0_prn_local_gru.pt";ck=torch.load(ckpath,map_location="cpu",weights_only=False);c=Cfg(**{k:ck["config"][k] for k in ("hidden_dim","emb_dim","dropout")});model=GRU(9,c);model.load_state_dict(ck["model_state_dict"])
 if ck["training_provenance"]["max_window_end_s"]>240:raise RuntimeError("CLIF B0 checkpoint includes post-train windows")
 ...
 for k in ("P0","P1","P2","P3"):
  ...
  for name,d in (("validation",val),("test",test)):
   yy,pp,meta=predict_model(d,models,lag,k);predrows.append({"split":name,"model":k,"mse":prediction_mse(yy,pp),"mae":mean_absolute_error(yy,pp),"samples":len(yy),"first_target_s":float(meta.t.min()),"last_target_s":float(meta.t.max()),"parameters":9 if k=="P0" else int(sum(np.size(v) for v in models[k].named_steps["ridge"].coef_))+int(np.size(models[k].named_steps["ridge"].intercept_)),"alpha":"N/A" if k=="P0" else alphas[k],"lag":lag})
 ...
 for shift in range(-6,7):
  x=val0.B0.to_numpy();y=pd.Series(val0.M1).shift(shift).to_numpy();ok=np.isfinite(y);lr.append({"alignment_shift_epochs":shift,"alignment_shift_s":shift*.5,"predictor_history_lag_epochs":lag,"predictor_history_lag_s":lag*.5,"validation_pearson":np.corrcoef(x[ok],y[ok])[0,1]})
 pd.DataFrame(lr).to_csv(out/"lag_analysis.csv",index=False);fig,ax=plt.subplots();ax.plot([x["alignment_shift_s"] for x in lr],[x["validation_pearson"] for x in lr],marker="o");ax.grid();fig.savefig(out/"plots/lag_analysis.png");plt.close(fig)
 ...
 # Provenance: source CSV/checkpoint hashes are computed live. Large attack-IQ digests are cached and explicitly not reverified.
 ...
 for s in ("cleanStatic",)+SCENARIOS:
  rp=RAW/("cleanStatic_gps.bin" if s=="cleanStatic" else f"{s}.bin")
  if s in canonical:
   rawrec={**file_record(rp,canonical[s],"cached canonical digest; current path/stat recorded but digest not reverified this run"),"digest_reverified_this_run":False}
  else:
   rawrec={**file_record(rp,"","live stat only; no trustworthy cleanStatic canonical hash available"),"sha256":None,"digest_reverified_this_run":False}
  prov.append({"scenario":s,"grade":"reconstructed" if s!="cleanStatic" else "provisional","raw_iq":rawrec,"sample_rate_hz":5000000,"sample_format":"interleaved little-endian int16 IQ","recording_start_sample":0,"seek_samples":0,"m1_block_ms":10,"stride_s":.5,"alignment":"timestamp reconstructed; receiver processing delay and independently authenticated common anchor unavailable","b0_source_csv":{**file_record(bpaths[s]),"digest_reverified_this_run":True},"m1_source_csv":{**file_record(mpaths[s]),"digest_reverified_this_run":True}})
 commit=subprocess.check_output(["git","rev-parse","HEAD"],cwd=ROOT,text=True).strip();deps={x:importlib.metadata.version(x) for x in ("numpy","pandas","scikit-learn","torch","matplotlib")};manifest={"schema":"clif-ip.r3.provenance.v3","generated_utc":datetime.now(timezone.utc).isoformat(),"source_commit_at_execution":commit,"dependencies":deps,"b0_checkpoint":{**file_record(ckpath),"digest_reverified_this_run":True,"training_provenance":ck["training_provenance"]},"scenarios":prov,"artifact_checksums":{}}
 ...
 cfg={"schema":"clif-ip.cross-layer.r3","splits":{"train":"whole containment [0,240] then reset transform/history","validation":"whole containment [250,330] then reset transform/history","independent_test":"whole containment [340,end] then reset transform/history"},"lag":lag,"alphas":alphas,"selection":"same lag candidates; common lag by mean validation MSE; separate validation alpha per P1/P2/P3; StandardScaler+Ridge; no attack labels","component_sets":COMPONENT_SPECS,"ablation_definitions":{"minus_M1":"B0 marginal + P1 residual only; no M1 scalar, P3, or concordance","minus_B0history":"M1 marginal + P2 residual only; no B0 marginal/history or concordance","minus_concordance":"B0 + M1 + P3 without concordance"},"onsets":ONSETS,"timing_basis":"available_s processing-time proxy; physical delay unavailable without receiver-delay/common-anchor verification","b0":"existing B0 architecture retrained/frozen on CLIF clean train","b0_training_max_window_end_s":ck["training_provenance"]["max_window_end_s"],"b0_checkpoint_sha256":sha(ckpath)};(out/"config.json").write_text(json.dumps(cfg,indent=2)+"\n")
 ...
 readme=f"""# CLIF-IP cross-layer R3 artifacts\n\n## Verdict: **{verdict}**\n\n1. **B0 freeze.** existing B0 architecture retrained/frozen on CLIF clean train: shared PRN-local GRU, signed nine-tap target, no PRN ID. Model and standardizer fit inputs are whole-contained in 0--240 s (max end {ck['training_provenance']['max_window_end_s']:.1f} s); the previously published B0 remains a separate comparator and was not modified.\n2. **M1 definition.** M1-style surrogate over actual raw-IQ features (not an original M1 checkpoint). Frozen hash `{state.sha256}`; exactly {audit.fit_count} clean fit and {state.fit_rows} whole-contained rows.\n3. **Leakage/reset.** Validation 250--330 s, independent clean test 340+ s, and os1--os4 are transform-only. Every split/recording resets M1/B0/downstream history, so B0 and all downstream states see no independent-test rows during fit.\n4. **Predictor protocol.** P0--P3 share signed nine-tap targets and support. Common validation-selected history lag={lag} epochs ({lag*.5:g} s); alphas {alphas}. `lag_analysis.csv` separately labels causal-alignment shifts and predictor-history lag.\n5. **Independent prediction.** P1 test MSE={p1:.9g}; P3={p3:.9g}; P3-vs-P1={100*(p1-p3)/p1:.3f}%. {'Improvement observed.' if p3<p1 else 'No incremental improvement; this is a failure for the cross-layer prediction claim.'}\n6. **Ablations/calibration.** Validation-only exact-set recalibration: Full=[B0,M1,P3,concordance], -M1=[B0,P1] (no M1/P3/concordance dependency), -B0history=[M1,P2] (no B0/concordance), -concordance=[B0,M1,P3].\n7. **Detection/alignment.** Full mean ROC-AUC={full.roc_auc.mean():.4f}, PR-AUC={full.pr_auc.mean():.4f}, independent clean-test FPR={full.independent_test_fpr.mean():.4f}. {a.permutations} permutations imply p-value resolution {1/(a.permutations+1):.6g}; reported 95% bounds are permutation-delta quantiles, not sampling CIs. `available_s` delays are processing-time proxies; physical delay cannot be claimed without receiver-delay/common-anchor verification.\n8. **Provenance/checksums.** B0/M1 CSV and checkpoint hashes were computed live. os1--os4 raw-IQ values are cached canonical digests; current path/stat was recorded but digest was not reverified this run. cleanStatic remains provisional/null. Final checksums (including `test_summary.txt`, excluding the manifest itself) are written only after tests.\n9. **Claim boundary/files.** No physical causality, exact sample alignment, original-M1 performance, cross-corpus generalization, or universal superiority claim. Bundle: `config.json`, `provenance_manifest.json`, `frozen_m1_fit_summary.json`, `clif_b0_prn_local_gru.pt`, `clif_b0_training_metadata.json`, `clif_b0_training_history.csv`, predictor/hyperparameter/scenario/fusion/ablation/lag CSVs, per-epoch CSVs, destruction JSON, plots, and `test_summary.txt`.\n""";(out/"README.md").write_text(readme)
