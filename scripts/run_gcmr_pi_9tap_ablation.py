...
from gnss_doppler_lab.gcmr_pi_9tap_adapter import index_peak_windows, build_event_record_indexed, CausalEventBuildError
from gnss_doppler_lab.gcmr_pi_9tap_pipeline import GCMRPeakInnovationPipeline
...
def main(argv=None):
 ap=argparse.ArgumentParser(description=__doc__);ap.add_argument('--output-dir',type=Path,default=Path('artifacts/gcmr_pi_9tap_ablation'));ap.add_argument('--history',type=int,default=4);ap.add_argument('--tap-count',type=int,choices=(9,),default=9);ap.add_argument('--epochs',type=int,default=30);ap.add_argument('--seed',type=int,default=7);ap.add_argument('--force-cache',action='store_true');ap.add_argument('--open-attacks',action='store_true');a=ap.parse_args(argv)
 ...
 try:
  ...
  if any(not built[k] for k in need):raise RuntimeError('GCMR relation events cannot be joined to real 9-tap windows')
