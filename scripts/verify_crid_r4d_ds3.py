#!/usr/bin/env python3
"""Fail-closed compact verifier for CRID R4d exploratory DS3 audit."""
...
REQUIRED = {
    ...
    "freeze_commit.json",
    "r4c_preserved_output_audit.json",
    "source_binding.json",
    "ds3_input_inventory.json",
    "replay_completion.csv",
    "empty_channel_inventory.csv",
    "support_audit.json",
...
def verify_manifest(artifact: Path) -> tuple[bool, list[str], dict]:
    ...
    try:
        ...
        if manifest.get("schema") != "gnss-doppler-lab.crid-r4d-artifact-manifest.v1" or manifest.get("status") != "PASS":
...
def verify(artifact: Path, mode: str = "auto") -> dict[str, object]:
    ...
    try:
        ...
        preserved = json.loads((artifact / "r4c_preserved_output_audit.json").read_text())
        empty_channels = read_csv(artifact / "empty_channel_inventory.csv")
    ...
    adapter = repair_prereg.get("manifest_adapter_repair", {})
    if not (
        repair_prereg.get("base_sha") == BASE_SHA
        and repair_prereg.get("authoritative_threshold") == THRESHOLD
        and repair_prereg.get("score_before_execution_repair_freeze") is False
        and adapter.get("missing_expected_file") == "FAIL_CLOSED"
        and adapter.get("empty_existing", {}).get("record_count") == 0
        and adapter.get("empty_existing", {}).get("size_bytes") == 0
        and adapter.get("empty_existing", {}).get("status") == "EMPTY_OPTIONAL_CHANNEL"
        and prereg_commit.get("status") == "PASS"
        and prereg_commit.get("ahead") == 0
        and prereg_commit.get("behind") == 0
    ):
        failures.append("repair_preregistration")
    if not (
    ...
    for binding in (*source.get("science_files", {}).values(), *source.get("frozen_configs", {}).values(), source.get("r4b_final_verdict", {}), source.get("r4b_source_binding", {}), source.get("r4b_artifact_manifest", {}), source.get("r4a_final_verdict", {}), source.get("r4c_final_verdict", {}), source.get("r4c_artifact_manifest", {}), source.get("r4c_source_binding", {}), source.get("source_handoff", {}), source.get("reanchored_handoff", {})):
    ...
    text = (artifact / "README.md").read_text(errors="replace") + json.dumps(final, sort_keys=True)
    ...
    if mode == "freeze":
        if replays or epochs or empty_channels:
            failures.append("pre_access_rows")
        if preserved.get("status") != "FROZEN_NOT_AUDITED" or preserved.get("audit_after_pushed_repair_freeze_only") is not True:
            failures.append("pre_access_r4c_audit")
        ...
        if preserved.get("status") not in {"RECOVERED_PRESERVED_C0", "HISTORICAL_INCOMPLETE_EVIDENCE"} or preserved.get("old_output_deleted_or_overwritten") is not False:
            failures.append("r4c_preserved_output_audit")
        for row in replays:
            if not (
                row.get("status") == "PASS"
            ...
            try:
                validate_completed_replay(Path(row["manifest_path"]), row["config"], inventory, source)
            except (OSError, KeyError, TypeError, ValueError, json.JSONDecodeError, RuntimeError):
                failures.append(f"replay_output_binding:{row.get('config')}")
        for row in empty_channels:
            if not (row.get("status") == "EMPTY_OPTIONAL_CHANNEL" and row.get("record_count") == "0" and row.get("size_bytes") == "0" and row.get("excluded_from_support") == "True" and _sha(row.get("sha256"))):
                failures.append(f"empty_channel:{row.get('config')}:{row.get('channel')}")
...
def _result(failures: list[str], mode: str, checks: dict[str, object]) -> dict[str, object]:
    return {
        "schema": "gnss-doppler-lab.crid-r4d-compact-verifier.v1",
        "mode": mode,
