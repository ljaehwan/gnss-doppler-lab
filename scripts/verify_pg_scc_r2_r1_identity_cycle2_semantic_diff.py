#!/usr/bin/env python3
"""Fail-closed semantic guard for the cycle-2 artifact-finalization ordering repair."""
...
ALLOWED_CHANGED_FUNCTIONS = {
    ...
    "main": "manifest creation moved immediately before required-output validation",
}
...
def _normalized_main_hash(node: ast.FunctionDef | ast.AsyncFunctionDef) -> tuple[str, bool]:
    parsed = ast.parse(ast.unparse(node)).body[0]
    assert isinstance(parsed, (ast.FunctionDef, ast.AsyncFunctionDef))
    names = [_call_name(statement) for statement in parsed.body]
    pairs = [
    ...
    if exact:
        index = pairs[0]
        parsed.body[index], parsed.body[index + 1] = parsed.body[index + 1], parsed.body[index]
    return _node_hash(parsed), exact
...
def _allowed_path(path: str) -> bool:
    return (
        ...
        or path.startswith("artifacts/pg_scc_stage0_r2_r1_identity_repair_cycle2/")
    )
...
def audit(base: str) -> dict[str, Any]:
    ...
    for name in sorted(set(before_functions) & set(after_functions)):
        ...
        exact_manifest_order = None
        if name == "main":
            after_hash, exact_manifest_order = _normalized_main_hash(after_functions[name])
            match = bool(exact_manifest_order and after_hash == before_hash)
            if not match:
                errors.append("main_not_exact_manifest_order_repair")
        function_report[name] = {
            ...
            **({"manifest_before_schema_validation": exact_manifest_order} if name == "main" else {}),
        }
    ...
    return {
        ...
        "schema": "pg_scc_stage0_r2_r1_identity_repair_cycle2_semantic_diff.v1",
...
def write_manifest(output: Path, base: str) -> None:
    ...
    payload = {
        ...
        "schema": "pg_scc_stage0_r2_r1_identity_repair_cycle2_implementation_manifest.v1",
    }
