...
def main(argv=None):
    ...
    training={"clean_node":str(node),"clean_manifest":str(man),"source_node_sha256":file_sha256(node),"checkpoint":str(ck),"checkpoint_sha256":actual,"rows":{k:len(v) for k,v in roles.items()},"audit":audit,"input_provenance":provenance,"historical_caveat":"CMTE-calibration-independent only; B0 trained across cleanStatic with PRN holdout"}
