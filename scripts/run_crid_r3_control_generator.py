...
from gnss_doppler_lab.crid_control_truth import write_truth_epochs
...
def main()->int:
 ...
 for case in cases:
  ...
  row['epoch_truth']=write_truth_epochs(ctx,case,alpha,out.parent/'truth_epochs.bin');side=out.parent/'truth.json';side.write_text(json.dumps(row,indent=2,sort_keys=True)+'\n');rows.append(row);print(json.dumps({'case':case.case_id,'sha256':row['output_sha256'],'status':'PASS'}),flush=True)
