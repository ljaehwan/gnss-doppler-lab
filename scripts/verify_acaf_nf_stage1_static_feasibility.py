...
EXPECTED={"ds3":{"pre":[148699,11],"onset_to_pulloff":[39762,6],"post_pulloff":[5200,3]},"ds4":{"pre":[107471,11],"post_onset":[18,1]},"ds7":{"pre":[100520,11],"post_onset":[0,0]},"ds8":{"pre":[102940,11],"110_to_150":[0,0],"ge_150":[288,1]}}
...
def verify(root:Path, recompute_external=True):
 ...
 try:
  ...
  expected_checksum_names=NAMES-{"checksums.json","verification_report.json"}
  if set(checks)!={"algorithm","files"} or checks.get("algorithm")!="sha256" or set(checks.get("files",{}))!=expected_checksum_names:errors.append("checksum_schema_or_inventory")
  elif any(digest(root/n)!=d for n,d in checks["files"].items()):errors.append("checksum_tamper")
  for name in CSVS:
  ...
  for name in ("normal_model_summary.json","thresholds.json","bootstrap_results.json"):
   if load(root/name).get("status")!="NOT_EVALUATED":errors.append("evaluated:"+name)
  ...
  for s,h in HASHES.items():
   ...
   if s!="cleanStatic" and (audits[s].get("counts")!=EXPECTED[s] or audits[s].get("matches_expected") is not True):errors.append("support_document:"+s)
  if EXPECTED["ds7"]["post_onset"][0] >= 20 or EXPECTED["ds8"]["110_to_150"][0] >= 20:errors.append("foundation_derivation")
 ...
 return {"status":"PASS" if not errors else "FAIL","derived_verdict":"FOUNDATION_INVALID" if not errors else None,"errors":errors,"independent_of_producer_verdict":True}

def main(argv=None):
 p=argparse.ArgumentParser();p.add_argument("artifact",type=Path);p.add_argument("--write-report",action="store_true");p.add_argument("--skip-external-recompute",action="store_true");a=p.parse_args(argv)
 result=verify(a.artifact,not a.skip_external_recompute)
 if a.write_report:(a.artifact/"verification_report.json").write_text(json.dumps(result,indent=2,sort_keys=True)+"\n")
