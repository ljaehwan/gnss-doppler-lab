#!/usr/bin/env python3
"""Fail-closed semantic guard for the PG-SCC R2 plot-only repair."""
...
ALLOWED_CHANGED_FUNCTIONS = {
    "_save_plots": "plot-only mixed-row schema filter",
    "verify_preregistration": "follow-up branch/base provenance gate",
    "verify_implementation_freeze": "follow-up branch/namespace provenance gate",
    "main": "single follow-up attempt-state provenance call only",
    "validate_committed_support_preflight": "follow-up committed-preflight namespace gate",
}
ALLOWED_NEW_FUNCTIONS = {
    "_write_followup_attempt_state": "follow-up provenance output only",
}
...
def _allowed_path(path: str) -> bool:
    return (
        ...
        or path.startswith("artifacts/pg_scc_stage0_r2_repair_followup/")
    )
...
def audit(base: str) -> dict[str, Any]:
    ...
    for name in sorted(set(before_functions) & set(after_functions)):
        ...
        if name == "main":
            normalized = _normalized_main_hash(after_functions[name])
            match = normalized == before_hash
            after_hash = normalized
    ...
    if function_report.get("_save_plots", {}).get("equivalent") is not False:
        errors.append("plot_function_not_changed")
    ...
    return {
        ...
        "schema": "pg_scc_stage0_r2_repair_followup_semantic_diff.v1",
...
def write_manifest(output: Path, base: str) -> None:
    ...
    payload = {
        ...
        "schema": "pg_scc_stage0_r2_repair_followup_implementation_manifest.v1",
    }
