#!/usr/bin/env python3
"""R2 CLI with all pre-result execution-environment repairs."""
...
from gnss_doppler_lab.qset_stage0a_r2_execution_repair_v4 import (
    bind_gnuradio_runtime,
    build_receiver_repaired_v4,
    clean_execution_repaired_v4,
    freeze_clean_artifacts_repaired_v4,
)
...
def main() -> int:
    ...
    if args.command == "build-receiver":
        result = build_receiver_repaired_v4()
    elif args.command == "run-clean":
        full = clean_execution_repaired_v4()
        freeze_clean_artifacts_repaired_v4(full)
        result = {
