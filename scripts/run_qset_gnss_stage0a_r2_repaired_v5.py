#!/usr/bin/env python3
"""R2 CLI with the pre-score Galileo VEML/empty-TRACE repair."""
...
from gnss_doppler_lab.qset_stage0a_r2_execution_repair_v5 import (
    activate_v5_receiver_paths,
    bind_gnuradio_runtime,
    build_receiver_repaired_v5,
    clean_execution_repaired_v5,
    freeze_clean_artifacts_repaired_v5,
)
...
def main() -> int:
    ...
    if args.command == "build-receiver":
        result = build_receiver_repaired_v5()
    elif args.command == "run-clean":
        full = clean_execution_repaired_v5()
        freeze_clean_artifacts_repaired_v5(full)
        result = {
