...
N_STRATUM_SUBSTANTIAL_SPREAD=.10
...
def main(argv=None):
    ...
    n_frame=pd.DataFrame(n_strata); eligible=n_frame[n_frame.epochs>=N_STRATUM_MIN_EPOCHS]
    n_spreads=eligible.groupby(["dataset","phase"]).full_epoch_occupancy.agg(lambda x:float(x.max()-x.min()))
    substantial_n_heterogeneity=bool((n_spreads>=N_STRATUM_SUBSTANTIAL_SPREAD).any())
    n_status=("dependence/heterogeneity observed: at least one eligible dataset/phase has >=0.10 occupancy spread across N strata; performance-N independence is not established" if substantial_n_heterogeneity else "inconclusive: one dataset/campaign cannot establish performance-N independence")
    mf=pd.DataFrame(metrics); mf.to_csv(out/"scenario_metrics.csv",index=False); pd.DataFrame(ablations).to_csv(out/"ablation_metrics.csv",index=False); pd.DataFrame(prn_summary).to_csv(out/"per_prn_evidence_summary.csv",index=False); n_frame.to_csv(out/"full_by_N_diagnostic.csv",index=False)
