#!/usr/bin/env python3
"""R2 CLI with Galileo TRACE/VEML loop-preservation repair."""
...
from gnss_doppler_lab.qset_stage0a_r2_execution_repair_v6 import (
    activate_v6_receiver_paths,
    bind_gnuradio_runtime,
    build_receiver_repaired_v6,
    clean_execution_repaired_v6,
    freeze_clean_artifacts_repaired_v6,
)
...
def main() -> int:
    ...
    if args.command == "build-receiver":
        result = build_receiver_repaired_v6()
    elif args.command == "run-clean":
        full = clean_execution_repaired_v6()
        freeze_clean_artifacts_repaired_v6(full)
        result = {
