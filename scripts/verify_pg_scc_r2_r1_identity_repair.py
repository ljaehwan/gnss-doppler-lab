#!/usr/bin/env python3
"""Verifier for the PG-SCC R2 r1 identity repair cycle."""
...
DEFAULT_ARTIFACT = ROOT / "artifacts/pg_scc_stage0_r2_r1_identity_repair_cycle1"
...
PREDECESSOR_ATTEMPT_STATE_SHA256 = "9203c3a998e7e92a320e6627cce799f6f008ea1721f14e1e8baa482ab163b4d0"
PLOTS = (
...
def verify_preexecution(root: Path, implementation_sha: str) -> dict[str, Any]:
    ...
    if errors:
        return {"schema": "pg_scc_stage0_r2_r1_identity_repair_preexecution.v1", "status": "FAIL", "errors": errors}
    ...
    if (
        ...
        or predecessor_metadata.get("protected_loader_invoked") is not False
    ...
    if (
        zero_access.get("protected_loader_invoked") is not False
        or zero_access.get("protected_score_fields_read") != 0
        or zero_access.get("protected_score_fields_projected_or_read") != 0
    ):
    ...
    for line in _git(repo, "status", "--porcelain=v1", "--untracked-files=all").splitlines():
        relative = line[3:]
        if relative.startswith("artifacts/pg_scc_stage0_r2_r1_identity_repair_cycle1/"):
            continue
        dirty.append(line)
    if dirty:
    ...
    return {
        ...
        "schema": "pg_scc_stage0_r2_r1_identity_repair_preexecution.v1",
...
def verify_tree(root: Path) -> dict[str, Any]:
    ...
    errors.extend(f"missing:{name}" for name in sorted(required) if not (root / name).is_file())
    if errors:
        return {"schema": "pg_scc_stage0_r2_r1_identity_repair_verification.v1", "status": "FAIL", "errors": sorted(set(errors))}
    prereg = _load(root / "preregistration.json")
    predecessor = _load(root / "predecessor_failure_binding.json")
    pre_run = _load(root / "pre_run_state.json")
    semantic = _load(root / "semantic_diff_audit.json")
    ...
    if (
        ...
        or metadata.get("protected_loader_invoked") is not False
    ):
        errors.append("predecessor_binding")
    if semantic.get("status") != "PASS" or not semantic.get("non_plot_scientific_functions_equivalent"):
        errors.append("semantic_diff")
    ...
    return {
        ...
        "schema": "pg_scc_stage0_r2_r1_identity_repair_verification.v1",
...
def verify_fresh_clone(root: Path, revision: str) -> dict[str, Any]:
    ...
    temp = Path(tempfile.mkdtemp(prefix="pg-scc-r2-r1-identity-verify."))
    try:
        clone = temp / "clone"
        ...
        command = [
            sys.executable, str(clone / "scripts/verify_pg_scc_r2_r1_identity_repair.py"),
            "--artifact", str(clone / "artifacts/pg_scc_stage0_r2_r1_identity_repair_cycle1"),
        ]
