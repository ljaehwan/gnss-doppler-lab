...
def run(args: argparse.Namespace) -> Path:
    ...
    output.mkdir(parents=True);raw_dir=output/"raw";raw_dir.mkdir()
    receiver_config=output/"receiver.conf";rewrite_config(Path(source["receiver_config_path"]),receiver_config,raw,raw_dir)
    ...
    if result.returncode!=0:raise RuntimeError(f"receiver failed with exit {result.returncode}")
    mats=sorted(raw_dir.glob("epl_tracking_ch_*.mat"));dats=sorted(raw_dir.glob("epl_tracking_ch_*.dat"))
    if not mats or len(mats)!=len(dats):raise RuntimeError("receiver did not produce paired MAT/DAT files")
    ...
    manifest={"schema":"acaf_nf_continuous_receiver_replay.v1","generated_at_utc":datetime.now(timezone.utc).isoformat(),
              "scenario":args.scenario,"source":{"path":str(raw),"sha256":source["raw_sha256"],"size_bytes":raw.stat().st_size,
              "sample_rate_hz":25_000_000,"sample_format":"ishort_complex_iq","first_file_sample":0},
              "receiver":{"command":command,"exit_code":result.returncode,"config":str(receiver_config),
